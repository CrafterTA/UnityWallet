    ...
    <div className={`min-h-screen ${isDark ? 'text-white' : 'text-slate-900'}`}>
      ...
      <div className="mb-8">
        <div className="text-center">
          ...
          <h1 className={`text-3xl font-bold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('insights.title', 'Financial Insights')}</h1>
          <p className={isDark ? 'text-white/70' : 'text-slate-600'}>{t('insights.subtitle', 'Track your spending patterns and discover smart savings opportunities with AI-powered insights')}</p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto space-y-6 px-6">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className={`text-lg font-semibold mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('insights.creditScore', 'Credit Score')}</h3>
          ...
          {spendingSummary?.monthly_trend && (
            <div className={`${isDark ? 'bg-white/5 border-white/10' : 'bg-white/80 border-slate-200'} backdrop-blur-xl rounded-2xl p-6 border`}>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('insights.monthlyTrend', 'Monthly Spending Trend')}</h3>
              </div>
        ...
        {insights?.recommendations && (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <p className={isDark ? 'text-white/70' : 'text-slate-600'}>{t('insights.recommendationsDesc', 'AI-powered insights to help you save more')}</p>
            </div>
        ...
        <div className="bg-gradient-to-r from-yellow-500/20 via-orange-500/20 to-red-500/20 backdrop-blur-xl rounded-2xl p-8 border border-white/10 shadow-2xl">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h3 className={`text-2xl font-bold mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('insights.specialOffer', 'Special Offer')}</h3>
              <p className={isDark ? 'text-white/70' : 'text-slate-600'}>{t('insights.specialOfferDesc', 'Exclusive deals just for you')}</p>
            </div>
          </div>
          <p className={`text-lg mb-6 leading-relaxed ${isDark ? 'text-white/90' : 'text-slate-700'}`}>
            {t('insights.travelOffer', 'Save 15% on travel bundles! Book flights + accommodation together using SkyPoints.')}
          </p>
          <button className="bg-red-600 text-white font-semibold py-3 px-6 rounded-xl hover:bg-red-700 transition-all duration-300 shadow-lg hover:shadow-xl">
