...
FastAPI endpoints for Sovico partner brands integration
"""

from fastapi import APIRouter, HTTPException, Query
from typing import Optional, List, Dict
from chain.services.sovico_mock import sovico_mock
...
async def get_user_points(user_id: str, brand_id: Optional[str] = Query(None)):
    ...
    try:
        points_data = await sovico_mock.get_user_points(user_id, brand_id)
        return {
            "success": True,
            "data": points_data
        }
...
async def redeem_points(request: Dict):
    ...
    try:
        result = await sovico_mock.redeem_points(
            user_id=request["user_id"],
            brand_id=request["brand_id"],
