...
from .schema import LoyaltyRequest, LoyaltyResponse, LoyaltyBalanceResponse
...
async def get_loyalty_balance(
    ...
    try:
        service = LoyaltyService(db)
        total_points = service.get_points_balance(str(current_user.id))
        return LoyaltyBalanceResponse(total_points=total_points)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
